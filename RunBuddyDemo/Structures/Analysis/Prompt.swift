...
struct Prompt {
    ...
    func newNutritionPrompt(with forecasts: [ChartForecast], name: String, duration: String, intensity: String) -> String? {
        ...
        let finalPrompt = """
                 ...
                 User: I am planning a \(intensity) intensity run for \(duration) minutes. I want nutritional advice on fueling with gels during the my run. How many gels should I consume and at what time interval? Also, what foods should I consume before and after my run? Provide a single paragraph response of 150 words or less. The general tone of the response should be upbeat, positive and encouraging. In your nutrition suggestions, consider the altitude of my location as well as weather forecast for heat and humidity. When required, only refer to yourself in first person. Do not use any titles or sub headings in the response. Numerical values provided in the response should be rounded to the nearest whole number. The response should also be in English.
                               
                  Location: \(name)
                      
                  Planned Exercise: Outdoor running
